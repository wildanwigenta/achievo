<div class="space-y-4 mt-6">
    @forelse ($tasks as $task)
        <div
            class="flex items-start justify-between bg-white px-5 py-4 border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition">
            <div class="flex items-start gap-3">
                <input type="checkbox" wire:click="toggleStatus({{ $task->id }})" {{ $task->status ? 'checked' : '' }}
                    class="mt-1">
                <div>
                    <h2
                        class="text-lg font-semibold {{ $task->status ? 'line-through text-gray-400' : 'text-gray-800' }}">
                        {{ $task->title }}
                    </h2>
